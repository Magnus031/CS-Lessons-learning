...
def make_withdraw(balance, password):
    ...
    def withdraw(amount,pw):
        nonlocal flag,balance,password,pw1
        if flag==3:
            return f"Too many incorrect attempts. Attempts: {pw1}"
        elif password==pw and flag<3:
            if amount>balance:
                return  'Insufficient funds'
            balance-=amount
            return balance
        elif pw!=password and flag<3:
            pw1.append(pw)
            flag+=1
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    if type(check)==str:
        return check
    '''The above case indicates that old_pass if the correct password and at the same time that the 
    account if have been locked if it has been locked propagate the error'''
