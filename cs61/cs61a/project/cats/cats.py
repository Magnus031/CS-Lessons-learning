...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for i in range(len(valid_words)):
        if user_word==valid_words[i]:
            return user_word
    p=[diff_function(user_word,valid_words[k],limit) for k in range(len(valid_words))]
    q=min(p)
    if q>limit:
        return user_word
    else:
        return valid_words[p.index(q)]
...
def time_per_word(times_per_player, words):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    num=len(times_per_player) #num is the number of players who have taken part in the race
    tpp=[]
    length=len(words)
    for i in range(num):
        time=[]
...
def fastest_words(game):
    ...
    fastest=[[] for i in range(num)]
    for i,word in enumerate(words):
        #enumerate函数返回的是一个tuple List （0,'apple'）,(1,'banana')云云
        word_times=[times[player][i] for player in range(num)]
        idx=min(range(num),key=lambda x:word_times[x])
