...
def balanced(m):
    ...
    if  is_planet(m):
        return True 
    if balanced(m[1][2])and balanced(m[2][2]) and total_weight(m[1][2])*m[1][1]==total_weight(m[2][2])*m[2][1]:
        return True 
    return False
def totals_tree(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    ''' the end only for 2 kinds: a mobile or a planet(base case)
    total_tree() is use recursive return total_tree(submobile) until the submobile is a planet
    '''
    if is_planet(m):
        return tree(size(m))
    else:
...
def quadratic(x, a, b, c):
    ...
    """
    "*** YOUR CODE HERE ***"
    mid=(lower_bound(x)+upper_bound(x))/2
    p=-b/(2*a)
    l=a*(lower_bound(x)**2)+b*lower_bound(x)+c
    r=a*(upper_bound(x)**2)+b*upper_bound(x)+c
    m=a*(p**2)+b*p+c
    if p<=upper_bound(x) and p>=lower_bound(x):
        return interval(min(l,r,m),max(l,r,m))
    else:
